...
// Update types to support both string and ColorTheme
type ColorType = string | ColorTheme;
...
const getThemeColor = (color: ColorType, theme: 'default' | 'dark'): string => {
...
export const Rating: React.FC<RatingProps> = ({
  ...
  size = 24,
  onChange,
  starColor = { default: '#ffd700', dark: '#ffa500' },
  hoverColor = { default: '#ffed4a', dark: '#ffb74d' },
  unratedColor = { default: '#e4e5e9', dark: '#424242' },
  ...
  const getCurrentStarColor = () => getThemeColor(starColor, theme);
  const getCurrentHoverColor = () => getThemeColor(hoverColor, theme);
  const getCurrentUnratedColor = () => getThemeColor(unratedColor, theme);
  ...
  const getColor = (index: number) => {
    if (hover !== null) {
      return index <= hover ? getCurrentHoverColor() : getCurrentUnratedColor();
    }
    const currentRating = hover !== null ? hover : rating;
    const difference = index - currentRating;
    if (difference > 0 && difference < 1) {
      return getCurrentStarColor();
    }
    return index <= rating ? getCurrentStarColor() : getCurrentUnratedColor();
  };
        ...
        {[...Array(maxRating)].map((_, index) => {
          ...
          return (
            <Star
              ...
              unratedColor={getCurrentUnratedColor()}
              style={{
