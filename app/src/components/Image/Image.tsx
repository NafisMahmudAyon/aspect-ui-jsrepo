...
export const Image: React.FC<ImageProps> = ({
  ...
  return (
    <>
      ...
      <figure
        className={`relative flex flex-col items-center justify-center ${responsive ? 'w-full h-auto' : ''} ${wrapperClassName}`}
      ...
      >
        <img
          ...
          `}
          style={{ width: width === 'full' ? '100%' : width === 'auto' ? 'auto' : `${width}px`, height: height === 'full' ? '100%' : height === 'auto' ? 'auto' : `${height}px` }}
        />
        ...
        {lightbox ? (
          ...
          >
            <span className={`text-accentColor dark:text-accentColor-dark text-caption ${previewTextClassName}`}>{previewText}</span>
          </div>
        ) : null}
        {caption && <figcaption className={`text-caption text-secondaryColor dark:text-secondaryColor-dark mt-2 ${captionClassName}`}>{caption}</figcaption>}
      </figure>

      {/* Lightbox Modal with fade-in/out effect */}
      {isLightboxOpen && (
        <div
          className={`fixed inset-0 overflow-auto my-6 bg-black bg-opacity-80 z-50 flex items-center justify-center transition-opacity duration-500 ${fadeIn ? 'opacity-100' : 'opacity-0'
            }`}
        >
          <figure className="relative max-w-5xl p-4">
            ...
            />
            {lightboxProps?.caption && <figcaption className={`text-sm text-gray-600 mt-2 ${captionClassName}`}>{(typeof lightboxProps.caption == "string") ? lightboxProps.caption : caption}</figcaption>}
            ...
            <div className="fixed top-6 right-6 space-y-2">
              ...
              <button
                ...
                className=" text-white size-10 flex items-center justify-center rounded-full leading-none shadow  text-lg"
              >
                <svg
                  fill="currentColor"
                  width="800px"
