...
export const Alert: React.FC<AlertProps> = ({ type, children, closeable = true, onClose, className='' }) => {
  ...
  return (
    <div className={cn(getAlertStyles(),className)} role="alert">
      ...
      {closeable && (
        <span className="absolute top-0 bottom-0 right-0 px-4 py-3" onClick={handleClose}>
          <svg className={`fill-current h-6 w-6 ${getCloseStyles()}`} role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            <title>Close</title>
