    ...
    const renderStar = (index) => {
        ...
        const currentColor = isHovered
            ...
            : isFilled
                ? starColor
                : unratedColor;
        return (_jsxs("div", { className: 'relative inline-block', style: { width: size, height: size, marginRight: 5 }, onMouseEnter: () => !readOnly && setHover(starValue), onMouseLeave: () => !readOnly && setHover(null), children: [_jsx(Star, { className: 'absolute left-0 top-0', color: currentColor, size: size }), !readOnly && (_jsx("input", { type: 'radio', name: 'rating', value: starValue, onClick: () => handleRatingChange(starValue), className: 'absolute left-0 top-0 h-full w-full cursor-pointer opacity-0' }))] }, index));
    };
