...
const Stepper = ({ children, initialStep = 0 }) => {
    ...
    return (_jsx(StepperContext.Provider, { value: contextValue, children: _jsx("div", { className: "flex items-center", children: React.Children.map(children, (child, index) => (_jsxs(React.Fragment, { children: [index > 0 && (_jsx("div", { className: `flex-grow h-0.5 mx-2 ${index <= activeStep ? 'bg-blue-500' : 'bg-gray-300'}` })), React.cloneElement(child, {
                        ...child.props,
                        index,
                    })] }, index))) }) }));
};
