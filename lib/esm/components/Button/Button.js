...
export const Button = ({ type = 'button', variant = 'primary', size = 'medium', disabled = false, loading = false, onClick, children, icon, iconPosition = 'left', className = '', iconClassName = '', 
// isFab = false,
position = 'bottom-right', ...rest }) => {
    const baseStyles = `${variant == 'link' ? "" : "inline-flex gap-2 items-center justify-center"} font-medium rounded-md focus:outline-none transition ease-in-out duration-200`;
    const sizeStyles = {
    ...
    // }
    return (_jsxs("button", { type: type, onClick: onClick, disabled: disabled || loading, className: cn(baseStyles, sizeStyles[size], variantStyles[variant], disabled ? 'cursor-not-allowed opacity-50' : '', loading ? 'relative' : '', 
        // isFab ? fabPositionStyles[position] : '',
        "font-sans", className), ...rest, children: [loading && (_jsx("span", { className: '', children: _jsxs("svg", { className: "animate-spin -ml-1 mr-3 h-5 w-5 text-white", xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", children: [_jsx("circle", { className: "opacity-25", cx: "12", cy: "12", r: "10", stroke: "currentColor", strokeWidth: "4" }), _jsx("path", { className: "opacity-75", fill: "currentColor", d: "M4 12a8 8 0 0 1 8-8V0C5.373 0 0 5.373 0 12zm2 5.291A7.96 7.96 0 0 1 4 12H0c0 3.042 1.135 5.824 3 7.938z" })] }) })), icon && iconPosition === 'left' && !loading && (_jsx("span", { className: `${iconClassName}`, children: icon })), !loading && children, icon && iconPosition === 'right' && !loading && (_jsx("span", { className: `${iconClassName}`, children: icon }))] }));
};
